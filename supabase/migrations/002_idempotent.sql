-- ============================================================
-- Sterova — Idempotent content schema (safe to re-run)
-- ============================================================

-- Patch admins: add is_active if missing
...
drop policy if exists "public_insert"        on public.reviews;
drop policy if exists "service_role_all"     on public.reviews;
create policy "public_read_approved" on public.reviews for select using (approved = true);
create policy "public_insert"        on public.reviews for insert with check (true);
create policy "service_role_all"     on public.reviews using (auth.role() = 'service_role');
create index if not exists reviews_created_at_idx on public.reviews (created_at desc) where approved = true;
...
create policy "public_read_active" on public.services for select using (is_active = true);
create policy "service_role_all"   on public.services using (auth.role() = 'service_role');
drop trigger if exists services_updated_at on public.services;
create trigger services_updated_at before update on public.services
  for each row execute procedure public.set_updated_at();
create index if not exists services_display_order_idx on public.services (display_order) where is_active = true;
...
('custom-software','Custom Software Development','Tailor-made software built around your exact business requirements.','We architect and build custom software solutions from the ground up — designed to fit your workflows, scale with your growth, and outlast off-the-shelf alternatives.','Code2',array['Requirements analysis and technical scoping','Full-stack development with modern frameworks','API design and third-party integrations','Automated testing and QA','CI/CD setup and deployment pipelines','Ongoing maintenance and support'],array['TypeScript','Node.js','PostgreSQL','React','Next.js'],1),
('web-development','Web Application Development','Fast, accessible, SEO-optimized web apps that convert and scale.','From marketing sites to full-featured SaaS platforms, we build web applications that are performant, accessible, and designed to grow.','Globe',array['Server-side and static rendering with Next.js','Responsive design for all screen sizes','SEO and Core Web Vitals optimization','Authentication, authorization, and user management','Real-time features and interactive UX','Analytics and conversion tracking setup'],array['Next.js','TypeScript','Tailwind CSS','Supabase','Vercel'],2),
('mobile-apps','Mobile App Development','Native-quality iOS and Android apps from a single codebase.','We build cross-platform mobile applications using React Native and Flutter — delivering native performance with efficient development cycles.','Smartphone',array['Cross-platform iOS and Android development','Native performance and smooth animations','Offline-first architecture','Push notifications and background services','App Store and Play Store publishing','OTA update support'],array['React Native','Flutter','TypeScript','Expo'],3)
...
drop policy if exists "public_read_active" on public.portfolio_items;
drop policy if exists "service_role_all"   on public.portfolio_items;
create policy "public_read_active" on public.portfolio_items for select using (is_active = true);
create policy "service_role_all"   on public.portfolio_items using (auth.role() = 'service_role');
...
create trigger portfolio_items_updated_at before update on public.portfolio_items
  for each row execute procedure public.set_updated_at();
create index if not exists portfolio_items_display_order_idx on public.portfolio_items (display_order) where is_active = true;
create index if not exists portfolio_items_featured_idx      on public.portfolio_items (is_featured)    where is_active = true;
...
drop policy if exists "service_role_all"   on public.testimonials;
create policy "public_read_active" on public.testimonials for select using (is_active = true);
create policy "service_role_all"   on public.testimonials using (auth.role() = 'service_role');
create index if not exists testimonials_display_order_idx on public.testimonials (display_order) where is_active = true;
...
drop policy if exists "public_read_active" on public.faqs;
drop policy if exists "service_role_all"   on public.faqs;
create policy "public_read_active" on public.faqs for select using (is_active = true);
create policy "service_role_all"   on public.faqs using (auth.role() = 'service_role');
create index if not exists faqs_display_order_idx on public.faqs (display_order) where is_active = true;
...
drop policy if exists "service_role_all" on public.site_settings;
create policy "public_read"      on public.site_settings for select using (true);
create policy "service_role_all" on public.site_settings using (auth.role() = 'service_role');
...
drop policy if exists "public_read_active" on public.navigation_items;
drop policy if exists "service_role_all"   on public.navigation_items;
create policy "public_read_active" on public.navigation_items for select using (is_active = true);
create policy "service_role_all"   on public.navigation_items using (auth.role() = 'service_role');
